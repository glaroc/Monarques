...
execGRASS('v.what.vect',map='selected_blocks',query_map='gtrs_10',query_column='grts_id',column='grts_id')
...
for (i in fids) {
  ...
  for (j in (1:6)) {
    ...
    if(npoints > 1){
      pts2<-pp.subsample(pts,n=npoints,gradient=0.2)
      writeOGR(pts2, "/home/ubuntu/data/Monarques/", "pts_temp", driver = "ESRI Shapefile", overwrite_layer=TRUE) 
      execGRASS('v.in.ogr',input="/home/ubuntu/data/Monarques/pts_temp.shp",output='temp3', flags=c('overwrite'))
      if(i==fids[1] && j==1){
...
```{r, eval=FALSE}
execGRASS('v.in.ogr',input='/home/ubuntu/data/Monarques/all30_with_cats.gpkg', output= 'all30_with_cats', flags=c('overwrite','o'))
